    ...
    public static async Task TagCommand(InteractionContext ctx,
        ...
        if (tag is null) { 
            await ctx.EditResponseAsync(new DiscordWebhookBuilder() {
                Content = $"Could not fetch url for {tagId}."
            });
            return;
        }
        ...
        if ((content.Length >= 4000 || (ephemeral && tag.IsPaged)) && !tag.IsEmbed) {
            if (ephemeral) {
                await ctx.EditResponseAsync(new DiscordWebhookBuilder() {
                    Content = $"Could not request tag as ephemeral."
                });
            }
            tag.IsPaged = true;
            tag.IsEmbed = true;
        ...
        IEnumerable<Page> pages = EagleBot.Interactivity.GeneratePagesInEmbed(content, SplitType.Line, new DiscordEmbedBuilder());
